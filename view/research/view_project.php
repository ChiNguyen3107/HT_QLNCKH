                            ...
                            <div class="row">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="file-attachment">
                                                ...
                                                <div class="flex-grow-1">
                                                    <h6 class="mb-1">File thuyết minh hiện tại</h6>
                                                    <p class="mb-0 text-muted"><?php echo htmlspecialchars($project['DT_FILEBTM']); ?></p>
                                                    <small class="text-muted">
                                                        Ngày tạo đề tài: <?php echo date('d/m/Y H:i', strtotime($project['DT_NGAYTAO'])); ?>
                                                    </small>
                                                </div>
                                                <?php 
                                                    $current_file_path = '';
                                                    if (strpos($project['DT_FILEBTM'], '/') !== false || strpos($project['DT_FILEBTM'], '\\') !== false) {
                                                        $web_path = preg_replace('#^\.\./\.\./#', '', str_replace('\\\\','/',$project['DT_FILEBTM']));
                                                        $current_file_path = '../../' . ltrim($web_path, '/');
                                                    } else {
                                                        $current_file_path = '../../uploads/project_files/' . $project['DT_FILEBTM'];
                                                    }
                                                ?>
                                                <a href="<?php echo htmlspecialchars($current_file_path); ?>" 
                                ...
                                <div class="col-md-6">
                                    <div class="card border-left-success">
                                        <div class="card-header">
                                            <h6 class="m-0 font-weight-bold text-primary">
                                                Tất cả file thuyết minh
                                            </h6>
                                        </div>
                                        <div class="card-body">
                                            <?php
                                            // Lấy lịch sử file thuyết minh từ database
                                            $proposal_history = [];
                                            ...
                                            while ($hist_row = $hist_result->fetch_assoc()) {
                                                $proposal_history[] = $hist_row;
                                            }
                                            // Nếu chưa có lịch sử nhưng đã có file hiện tại từ khi đăng ký, thêm vào danh sách
                                            if (empty($proposal_history) && !empty($project['DT_FILEBTM'])) {
                                                $proposal_history[] = [
                                            ...
                                            if (empty($proposal_history)) {
                                            $upload_dir = '../../uploads/project_files/';
                                            if (is_dir($upload_dir)) {
                                                $files_in_dir = scandir($upload_dir);
                                                foreach ($files_in_dir as $file) {
                                                    if ($file != '.' && $file != '..' && $file != '.htaccess' && $file != 'README.md') {
                                                        // Kiểm tra xem file có liên quan đến đề tài này không
                                                        if (strpos($file, $project_id) !== false || 
                                                            (strpos($file, 'proposal_') !== false && strpos($file, '.docx') !== false)) {
                                                                $proposal_history[] = [
                                                                    'FILE_TEN' => $file,
                                                                    'FILE_KICHTHUOC' => file_exists($upload_dir . $file) ? filesize($upload_dir . $file) : null,
                                                                    'FILE_LOAI' => pathinfo($file, PATHINFO_EXTENSION),
                                                                    'LY_DO' => 'File thuyết minh (tìm thấy trong thư mục)',
                                                                    'NGUOI_TAI' => 'Hệ thống',
                                                                    'NGAY_TAI' => file_exists($upload_dir . $file) ? date('Y-m-d H:i:s', filemtime($upload_dir . $file)) : date('Y-m-d H:i:s'),
                                                                    'LA_HIEN_TAI' => 0
                                                                ];
                                                            }
                                                        }
                                            ...
                                            usort($proposal_history, function($a, $b) {
                                                return strtotime($b['NGAY_TAI']) - strtotime($a['NGAY_TAI']);
                                            });
                                            ?>
                                            <?php if (count($proposal_history) > 0): ?>
                                            <?php foreach ($proposal_history as $index => $file): ?>
                                            <div class="file-attachment">
                                                <div class="file-icon">
                                                </div>
                                                <div class="flex-grow-1">
                                                    <h6 class="mb-1">
                                                        <?php echo htmlspecialchars($file['FILE_TEN']); ?>
                                                        <?php if ($file['LA_HIEN_TAI']): ?>
                                                            <span class="badge badge-success ml-2">Hiện tại</span>
                                                        <?php endif; ?>
                                                    </h6>
                                                    <p class="mb-0 text-muted">
                                                        <?php echo htmlspecialchars($file['LY_DO']); ?> | 
                                                        <?php echo date('d/m/Y H:i', strtotime($file['NGAY_TAI'])); ?>
                                                        <?php if (!empty($file['NGUOI_TAI'])): ?>
                                                            | <?php echo htmlspecialchars($file['NGUOI_TAI']); ?>
                                                        <?php endif; ?>
                                                    </p>
                                                    <?php if (isset($file['FILE_KICHTHUOC']) && is_numeric($file['FILE_KICHTHUOC'])): ?>
                                                    <small class="text-muted">
                                                        Kích thước: <?php echo number_format($file['FILE_KICHTHUOC']); ?> bytes
                                                    </small>
                                                    <?php endif; ?>
                                                </div>
                                                <?php 
                                                    $file_path = '';
                                                    if (strpos($file['FILE_TEN'], '/') !== false || strpos($file['FILE_TEN'], '\\') !== false) {
                                                        $web_path = preg_replace('#^\.\./\.\./#', '', str_replace('\\\\','/',$file['FILE_TEN']));
                                                        $file_path = '../../' . ltrim($web_path, '/');
                                                    } else {
                                                        $file_path = '../../uploads/project_files/' . $file['FILE_TEN'];
                                                    }
                                                ?>
                                                <a href="<?php echo htmlspecialchars($file_path); ?>" 
                                                   class="btn btn-sm btn-outline-success" target="_blank" download>
                                                </a>
                                            </div>
                                            <?php endforeach; ?>
                                            <?php else: ?>
                                            <p class="text-center text-muted">Chưa có file thuyết minh nào.</p>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
