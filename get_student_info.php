...
// Kiểm tra đăng nhập - cho phép truy cập nếu có session hoặc đang ở trang register
$is_logged_in = isset($_SESSION['MANGUOIDUNG']) || isset($_SESSION['user_id']);
$is_register_page = isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'register_project_form.php') !== false;

if (!$is_logged_in && !$is_register_page) {
...
try {
    ...
    error_log('Lỗi get_student_info.php: ' . $e->getMessage());
    echo json_encode([
        'success' => false, 
